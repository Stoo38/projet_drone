...
PROJECT_PATH="/tp/xph3app/xph3app602/DE1_SOC_CAMERA_IP_Vhdl"		# Path of the Project Directory
...
then
	...
	vmap ${LIB_NAME} ${PROJECT_PATH}/LIBS/${LIB_NAME}					>> ${PROJECT_PATH}/LOG/${LOG_FILE}	# Creates a link between the name of the library and its path
		...
		vcom ${OPT_VLOG} ${LIB_NAME} /softslin/altera14_0/quartus/eda/sim_lib/altera_lnsim_components.vhd	>> ${PROJECT_PATH}/LOG/${LOG_FILE}	# Compile all the Verilog files contained on the VHDL_FILES variable

		vcom ${OPT_VLOG} ${LIB_NAME} /softslin/altera14_0/quartus/eda/sim_lib/altera_primitives_components.vhd	>> ${PROJECT_PATH}/LOG/${LOG_FILE}	# Compile all the Verilog files contained on the VHDL_FILES variable

		vcom ${OPT_VLOG} ${LIB_NAME} /softslin/altera14_0/quartus/eda/sim_lib/cyclonev_components.vhd	>> ${PROJECT_PATH}/LOG/${LOG_FILE}	# Compile all the Verilog files contained on the VHDL_FILES variable
																															

	for file in ${VHDL_FILES}
	do
		vcom ${OPT_VCOM} ${LIB_NAME} ${PROJECT_PATH}/vhdl/${file}		>> ${PROJECT_PATH}/LOG/${LOG_FILE}	# Compiles all the VHDL files contained on the VHDL_FILES variable
